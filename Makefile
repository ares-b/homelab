...
CONFIG_SOPS       := $(CURDIR)/config.sops.yaml
ANSIBLE           ?= ansible-playbook
SOPS_EXEC         := python3 $(CURDIR)/scripts/sops-exec.py
SOPS_ANSIBLE      := python3 $(CURDIR)/scripts/sops-ansible.py
SSH_PRINCIPAL     ?= $(USER)
...
images:
	cd packer && sops exec-file $(CONFIG_SOPS) '$(SOPS_EXEC) {} PKR_VAR_ packer ./deploy.sh $(PACKER_ARGS)'

image-k3s:
	cd packer && sops exec-file $(CONFIG_SOPS) '$(SOPS_EXEC) {} PKR_VAR_ packer ./deploy.sh -only="ubuntu-k3s.proxmox-iso.ubuntu-k3s" $(PACKER_ARGS)'

image-docker:
	cd packer && sops exec-file $(CONFIG_SOPS) '$(SOPS_EXEC) {} PKR_VAR_ packer ./deploy.sh -only="ubuntu-docker.proxmox-iso.ubuntu-docker" $(PACKER_ARGS)'
...
garage-plan garage-apply garage-destroy:
	...
	PF_PID=$$!; sleep 2; \
	cd $(GARAGE_TF_DIR) && sops exec-file $(CONFIG_SOPS) '$(SOPS_EXEC) {} TF_VAR_ garage terraform $(subst garage-,,$@) -auto-approve'; \
...
pve-bootstrap: | $(INFRA_CA_FILE)
	CA='$(INFRA_CA)' sops exec-file $(CONFIG_SOPS) \
		'$(SOPS_ANSIBLE) {} pve_bootstrap \
		sh -c "cd $(CURDIR)/pve-bootstrap && ansible-playbook site.yml \
		-i inventory -e @{section_vars} \
		-e \"pve_security_ssh_ca_public_key=$$CA\""'
